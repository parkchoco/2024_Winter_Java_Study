...
public class DynamicBeat extends JFrame{//JFrame은 GUI 기반의(그래픽) 프로그램을 만들기 위한 기본적으로 상속 받아야하는 부분
	...
	public DynamicBeat() {
		...
		Music introMusic = new Music("introMusic.mp3", true);// true로 설정해주었기 때문에 음악을 직접 종료시켜주기 전에는 계속 반복해서 재생된다.
		//시작화면에서 음악이 무한정 반복해서 재생이 이루어지도록 해준다.
		introMusic.start();//게임이 실행됨과 동시에 시작화면에 인트로 뮤직이 실행된다.
	}
